...
server <-function(input,output){
  ...
  sliderValues <- reactive({
    ...
    output$outcom <- renderText({
      "show the The calculation results"
    })

    output$N <- renderText({
      k=input$K
      gamma=input$delta1/input$delta2
      rho<-matrix(c(1,input$rho12,input$rho12,1),ncol=input$K)
      ck<-CKsolution(input$alpha,input$power,rho,gamma,k)
      paste("The sample size of your clinical trail is:",Nsolution(input$alpha,input$r,input$delta2,ck))
    })
